...
# Validate keys — answer relevancy only needs question and answer
...
# Summary
avg_relevancy = sum(r["score"] for r in results) / len(results) if results else 0
print(f"\nEvaluated: {len(results)}/{len(test_cases)} samples ({len(failed)} failed)")
print(f"Final Average Answer Relevancy: {avg_relevancy:.2f}")